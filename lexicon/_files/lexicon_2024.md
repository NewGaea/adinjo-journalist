# List of Words (2024)

- [List of Words (2024)](#list-of-words-2024)
  - [A](#a)
  - [B](#b)
  - [G](#g)
  - [D](#d)
  - [E](#e)
  - [Z](#z)
  - [H](#h)
  - [C](#c)
  - [I](#i)
  - [K](#k)
  - [Q](#q)
  - [L](#l)
  - [M](#m)
  - [N](#n)
  - [J](#j)
  - [P](#p)
  - [R](#r)
  - [S](#s)
  - [T](#t)
  - [U](#u)
  - [Y](#y)
  - [F](#f)
  - [V](#v)
  - [X (KH)](#x-kh)
  - [O](#o)
  - [W](#w)
  - [OLD FORMS](#old-forms)
  - [INCOMPLETE](#incomplete)
  - [UNSORTED](#unsorted)

This list contains only words which are confirmed as added to the lexicon documentation in 2023 and 2024, even if they are older.

## A

- [x] **adinjo** 1. (_proper noun_) Adin language `#2023.01.10`
- [x] **aimijálun** 1. (_noun_) blindfold, eye patch (att. 2023-Jan-28)
- [x] **alnogalmi** 1. (_descriptor_) lawful, legal, legitimate (att. 2023-Jan-20_)
- [x] **andévo** 1. (_noun_) brotherhood, fraternity (att. 2023-Feb-03)
- [x] **ando** 1. (_noun_) cave, cavern (added 2023-Jan-13)
- [x] **ancoté** 1. (_noun_) sisterhood, sorority (att. 2023-Feb-03)
- [x] **asenkuha** 1. (_noun_) dawn, sunrise (att. 2023-Jan-21)
- [x] **ashenros** 1. (_noun_) hypnosis, meditation, trance (att. 2023-Jan-21)
- [x] **aur.en** 1. (_ton verb_) asking, interrogating, querying, questioning `#2023.01.10`

## B

- [x] **banbor.ak** (ban.'bor.ak) 1. (_ton verb_) to buy, to trade; a compound of _bancels_ (giving) and _borak_ (getting) (coined 2023-Sep-08)
- [x] **bandomi** 1. (_noun_) bakery, bread store, patisserie (att. 2023-Jan-28)

## G

- [x] **gázi** 1. (_noun_) a board or plank of wood (att. 2023-Jan-05)
- [x] **gendaxin** 1. (_noun_) childhood, youth (att. 2023-Jan-20)
- [x] **gramurnau** 1. (_descriptor_) gramurn; of or related to the Gramurn (added 2023-Jan-13)
- [x] **gramurjo** 1. (_noun_) gramurn language (added 2023-Jan-13)
- [x] **gormachin** 1. (_noun_) adjective, adverb; the class of all descriptive words (att. 2023-Jan-22)
- [x] **goron** 1. (_noun_) addition (att. 2023-Jane-13)

## D

- [x] **dahanau** 1. (_descriptor_) poisonous, toxic, venomous (att. 2023-Jan-13)
- [x] **dakmelikin** 1. (_noun_) character set, script, writing system (att. 2023-Jan-29)
- [x] **dama.i** 1. (_ton verb_) thanking (att. 2023-Jan-13)
- [x] **dama.rio** 2. _also **damarion, damarionu**_ (_noun, descriptor_) happenings, news, new event, news story (cn. 2023-May-27)
- [x] **dapiyani.s** 1. (_ton verb_) enunciating (att. 2023-Jan-13)
- [x] **del nanda** 1. (_pronoun_) how many, how numerous, what amount (att. 2023-May-07)
- [x] **dévo** 1. (_noun_) brother, brothers (att. 2023-Feb-03)
- [x] **dìn** 2. (_descriptor_) repetitions, sets of, times (att. 2023-Jan-20)
- [x] **dudama** 1. (_descriptor_) newer (att. 2023-Jan-20)
- [x] **duralmen** 1. (_noun_) cream (att. 2023-Jan-20)
- [x] **dyurama** 1. (_descriptor_) newest (att. 2023-Jan-20)
- [x] **do'ìníja** 1. (_descriptor_) not safe for work, unwise for work (a contracted form of _dobra inda ijina_; att. 2023-Jan-28)
- [x] **doramau** 1. (_descriptor_) decayed, rotten, spoiled (att. 2023-Jan-23; derived from _doram.u_ => decaying, rotting, spoiling)

## E

- [x] **ellonemal** 1. (_noun_) display, monitor, screen (att. 2023-Jan-20)
- [x] **éresh** 1. (_noun_) fang, tooth (att. 2023-Jan-23, derived from _kénaresh_ => canine tooth, incisor) 2. bead (att. 2023-Jan-23)

## Z

- N/A

## H

- N/A

## C

- [x] **chennur** (gen. 2022-Jun-23) 1. (_noun_) academy, college, school, university (att. 2023-Jan-05) 2. (_noun_) the group of people who follow a set of teachings (att. 2023-Jan-05)
- [x] **chennuri** 1. (_descriptor_) learned, educated, smart (att. 2023-Jan-05)
- [x] **cocibi** 1. (_descriptor_) nightly (att. 2023-Jan-29)
- [x] **coté** 1. (_noun_) sister, sisters (att. 2023-Feb-03)

## I

- [x] **immenachin** 1. (_noun_) epithet, euphemism, pronoun, pro-verb; the class of words which are able to refer back to a previous referent (att. 2023-Jan-22)

## K

- [x] **karnkuha** 1. (_noun_) dusk, sunset (att. 2023-Jan-21)
- [x] **kajac.en** 1. (_noun_) twisting, turning (att. 2023-Feb-03) 2. (_ton verb_) squirming, writhing (att. 2023-Feb-03)
- [x] **kafa** 1. (_noun_) coffee, tea (att. 2023-Jan-28)
- [x] **kafadomi** 1. (_noun_) café, coffee shop, tea shop (att. 2023-Jan-28)
- [x] **kendalin** 1. (_descriptor_) epic, heroic, fantastic, legendary
- [x] **kérinjia** 1. (_noun_) yesterday; the recent past (att. 2023-Jan-21)
- [x] **kikíten** 1. (_noun, casual_) puppyboy 2. (_noun, slang_) tomgirl, twink (att. 2023-Jan-21)
- [x] **kira.ja** 1. (_ton verb_) pulling, removing (att. 2023-Feb-03)
- [x] **kudja** 1. (_noun_) cipher, coded message, rebus (att. 2023-Jan-29)
- [x] **kudjani** 1. (_noun_) cipher, encoding, encryption (att. 2023-Jan-29)
- [x] **kuhalorek** 1. (_noun_) sunlight, sunshine (att. 2023-Jan-21)
- [x] **kybralirja** 1. (_noun_) email, online message (att. 2023-Jan-29)
- [x] **kolvan** 1. (_noun_) finger, toe (att. 2023-Jan-23)

## Q

- [x] **qarinban** (kʷa.'rin.ban) 1. (_noun_) turnover, empanada, pie; literally "filling bread," a small loaf of bread or pastry with various other ingredients cooked into it, often sold as street food at the storefronts of bakers or restaurants (coined 2023-Sep-07)
- [x] **qejinchin** 1. (_noun_) positional, postposition, preposition; the class of words which define a place in space or time (att. 2023-Jan-22)

## L

- [x] **larntin** 1. (_noun_) likeness, appearance, image, something having a similar or identical sensation to something else (att. 2023-Jan-05) 2. (_noun_) picture, drawing, painting, sculpture (att. 2023-Jan-05)
- [x] **larnoc** 1. (_noun_) double-mug, grande, medium, pint (att. 2023)
- [x] **latèn** 1. (_noun_) an illustration of realistic style or quality (att. 2023-Jan-05)
- [x] **ligorem damarion** 1. (_noun_) news broadcast (literally "broadcast (about) happenings"; cn. 2023-May-27)
- [x] **lisvaln** 1. (_noun_) xylophone (derived from _lici + ufaln_ "the sound of knocking sticks"; att. 2023-Jan-13)

## M

- [ ] **mékaput** 1. (_noun_) MILF, shortened form of _medra yi kazade putochi_ (att. 2023-Jan-20)
- [ ] **menachin** 1. (_noun_) label, noun, title; the class of naming words (att. 2023-Jan-22)
- [ ] **mokaurok** 1. (_noun_) atelier, office, studio, study (room), workshop (att. 2023-Jan-21)
- [ ] **mokuretax** 1. (_noun_) apprentice, disciple, follower, student (att. 2023-Jan-21)
- [ ] **mornai** 1. (_noun_) girl, young lady, young woman (att. 2023-Jan-20)
- [ ] **mornen** 1. (_noun_) boy, young man (att. 2023-Jan-20)
- [ ] **morstan** 1. (_noun_) biography (att. 2023-Jan-29)
- [ ] **morxagi** 1. (_noun_) a sapling or cutting of a tree intended for planting or grafting (att. 2023-Jan-05)

## N

- [ ] **naerkust** 1. (_noun_) hedge wizard, shaman, witch (att. 2023-Jan-20)
- [ ] **nandé.o** 1. (_ton verb_) counting, enumerating, listing, numbering (att. 2023-Jan-23; derived from _nanda_ => number)
- [ ] **néter** (sometimes **néther**) 1. (_noun_) the nether (in Minecraft) (att. 2023-Jan-05)
- [ ] **néterak** (sometimes **nétherak**) 1. (_noun_) netherrack (in Minecraft) (att. 2023-Jan-05)
- [ ] **nocash** 1. (_noun_) sandstone (att. 2023-Jan-05)
- [ ] **nomori** 1. (_noun_) beverage, drink (att. 2023-Jan-28)
- [ ] **nonkelz** 1. (_noun_) glacier, iceberg (att. 2023-Jan-21)

## J

- [ ] **jálban** (ʒæl.ban) 1. (_noun_) wrap, burrito, calzone, dumpling; a dish composed of bread wrapped around fillings, from _jálust_ (wrapping) and _ban_ (bread) (coined 2023-Sep-07)
- [ ] **jalu'ai** 1. (_descriptor_) dirty, filthy, soiled (att. 2023-Jan-23; derived from _jalu_ => dirt)
- [ ] **jama.r** (gen. 2022-Jun-23) 1. (_ton verb_) to flee, to run, to escape (att. 2023-Feb-17\*)
- [ ] **jesodomya** 1. (_noun_) clinic, hospital, medical centre (att. 2023-Jan-29)
- [ ] **ju** 2. (_descriptor_) live, current, in-progress; especially when added as a prefix to a verb (att. 2023-Jan-09)
- [ ] **julanjyu** 1. (_descriptor_) transgender (att. 2023-Jan-21)
- [ ] **julanjyudin** 1. (_noun_) transgender person (att. 2023-Jan-21)
- [ ] **julígorem** 1. (_gerund_) livestream, live broadcast; does not conjugate on its own, but can be followed by _kempo_ to make verbal constructions (att. 2023-Jan-05)
- [ ] **juju** 1. (_positional_) at this time, immediate, instant, right now (cn. 2023-May-25)
- [ ] **joma** 1. (_noun_) the trunk of a tree (att. 2023-Jan-05) 2. (_noun_) a length of a felled tree trunk or broad branch (att. 2023-Jan-05)
- [ ] **jonauket** 1. (_noun_) threesome; a physically intimate sex act involving three participants

## P

- [ ] **paratwan damarion** 1. (_noun_) newsletter, newspaper (literally "letters (about) happenings"; cn. 2023-May-27)
- [ ] **paurenen** 1. (_noun_) bad boy, gambler, risk-taker 2. (_noun_) playboy
- [ ] **polchotax** 1. (_noun_) deceiver, oath breaker 2. (_noun_) demonist, warlock

## R

- [ ] **rishemat** 1. (_noun_) cottage, farmhouse (att. 2023-Jan-28)
- [ ] **ronoci** 1. (_noun_) redstone (in Minecraft) (att. 2023-Jan-05)

## S

- [ ] **sorin** 1. (_noun_) older sister (att. 2023-Jan-17)
- [ ] **stribó** (gen. 2022-Jun-20) 1. (_noun_) ore, stone containing valuable minerals or gems (att. 2023-Jan-05)

## T

- [ ] **tabuk** 1. (_noun_) older brother (att. 2023-Jan-17)
- [ ] **tan** 1. (_noun_) myth, story, tale (att. 2023-Jan-29, existed by 2003-Dec-25 in _furatan_ "lemon story")
- [ ] **temborel** 1. (_noun_) starting then, thereafter (att. 2023-Jan-21)
- [ ] **téomokurend** 1. (_noun_) divinities, theology (att. 2023-Jan-21)
- [ ] **titan** 1. (_noun_) colossus, giant, golem, titan (att. 2023-Jan-21)
- [ ] **titanau** 1. (_descriptor_) colossal, gigantic (att. 2023-Jan-21)
- [ ] **toxma** 1. (_noun_) loss, defeat (att. 2023-Jan-20)

## U

- N/A

## Y

- [ ] **yanda** 1. (_noun_) cry, call, speech, voice (att. 2023-Jan-20)
- [ ] **yevjálun** 1. (_noun_) gag, muzzle (att. 2023-Jan-28)
- [ ] **yevihauk.ret** 1. (_ton verb_) bobbing, blowing, licking, sucking; performing sensual acts with the mouth and tongue, especially oral sex (att. 2023-Jan-28)
- [ ] **yevihauket** 1. (_noun_) blowjob, cunnilingus; any act of oral sex (att. 2023-Jan-28)

## F

- N/A

## V

- [ ] **varchin** 1. (_noun_) conjunction; the class of words that connect words, phrases, and clauses (att. 2023-Jan-22)
- [ ] **ventroganwas** 1. (_noun_) passport, travel papers, visa (att. 2023-Jan-20)
- [ ] **vibun** (gen. 2022-Jun-23) 1. (_noun_) iron (metal) (att. 2023-Jan-05)

## X (KH)

- [ ] **xálic** 1. (_noun_) rod, staff, stick, wand (att. 2023-Jan-20)
- [ ] **xáu** 1. (_descriptor_) magical (att. 2023-Jan-20)
- [ ] **xlamastrai** 1. (_noun_) converter, interface, translator (att. 2023-Jan-20)

## O

- [ ] **orom** 2. (_descriptor_) equal (att. 2023-08-13)

## W

- N/A

## OLD FORMS

The words listed in this section may be repurposed and redefined as they have modern forms in the lexicon.

- **bura.nu** _ton verb_ to blow, to exhale (att. 2003-Dec-25) _has become modern "brau.jen"_
- **imá.ton** _ton verb_ to shoot (att. 2003-Dec-25 ${\rightarrow}$ 2022-Jun-05) _has become modern "íma.ta"_

## INCOMPLETE

- **'e.tol** _ton verb_ to burn, to be passionate, to be moved to action (att. 2007-Nov-26; ret. 2022-Jun-05) _conjugated, and contracted, form not typically listed, but no other form appears in the song where this word is attested_
- **soli.tol** _ton verb_ `#UNKNOWN_GLOSS` (att. 2003-Dec-25; ret. 2022-Jun-05) _conjugated, form not typically listed, but no other form appears in the song where this word is attested_

## UNSORTED

- N/A
- **amuran** 1. (_noun._) spring, warming season; (c. 2023-Dec-08; from _amurent_ "scattering, sowing")
- **gir** 1. (_desc._) warm, hot; a good state for a fresh cooked meal or brewed drink to be in (gen. 2022-06-23, c. 2023-Dec-08)
- **girina** 1. (_noun._) summer, warm season, hot-dry season (gen. 2022-06-23, c. 2023-Dec-08)
- **amkeiban** 1. (_noun._) sliced bread; a slice of bread 2. sandwich, filling served between two or more slices of bread